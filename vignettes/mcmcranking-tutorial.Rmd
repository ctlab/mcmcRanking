...
`repetition_depth` function calculates number of iteration, where it finds smallest number $n$ where $2^n$-th root of maximum likelihood devided minimum likelihood value less than $4$.
```{r message=FALSE}
...
y <- sample_llh(graph = g, module_size = 1, niter = 1e5, exp_lh = 1/2^depth)
...
We found out how many iteration enough for convergence of *MCMC* method. Below function `mcmc_sample` iterativly runs *MCMC* with different likelihood values. We sample $10^3$ subgraphs using *MCMC*.

```{r message=FALSE, eval=FALSE}
z <- mcmc_sample(graph = g, module_size = 1, times = 1e3, niter = 2e4, exp_lh = 1/2^(depth:0))
```
